...
try:
    ...
    print("""Einige Module konnten nicht importiert werden. Um dieses Skript zu starten, sollte zunÃƒÂ¤chst der Befehl "source bin/activate" (oder poetry shell o.ÃƒÂ¤.) in der Kommandozeile eingegeben werden.""")
    raise
    #sys.exit(1)
...
#    mask[-100:,:]=False
    ...
    logger.debug(f'LÃƒÂ¶sche {nrow} Zeilen und {ncol} Spalten.')
    ...
    if len(im_cropped.shape)==2:
        ...
        raise AssertionError('Dieser Fall wurde noch nicht beachtet. Nur Graustufen- und Farbbilder werden unterstÃƒÂ¼tzt.')
...
def generate_mask_with_hsv_threshold(im, sign=None,hue_threshold=None, saturation_threshold=None, value_threshold=None):
    ...
    elif im.dtype not in (np.float32, np.float64):
...
def create_mask(im,fraction_of_rows_to_remove=0.0,fraction_of_cols_to_remove=0.1,hue_threshold=None,saturation_threshold=None,value_threshold=100,erosion_size=5,dilation_size=50,value_to_fill=0):
    ...
    #Passe die Maske mit morphologischen Transformationen an, insbesondere, um vereinzelte Pixel zu eliminieren (Erosion) und ev. um die Maske leicht zu vergrÃƒÂ¶ssern (Dilatation)
...
def insert_region_number(basename,ireg):
    """
    GemÃƒÂ¤ss Konvention kÃƒÂ¶nnen wir Dateinamen der Form <irgendwas>_<Klassenname>.jpg haben. 
...
def show_region(region,ax=None):
    ...
    if ax is None:
        ...
        plt.title(f'Orientation: {(180*orientation/np.pi):1.0f}Ã‚Â°',va='center')

    else:
        ax.plot(bx, by, '-b', linewidth=2.5)
        ax.set_title(f'Orientation: {(180*orientation/np.pi):1.0f}Ã‚Â°',va='center')
...
def process_file(fn,fraction_of_rows_to_remove=0,fraction_of_cols_to_remove=0,
    ...
    """
    Objektextraktion aus einer Datei. <fn> ist eine Bilddatei. Die SchlÃƒÂ¼sselwortargumente werden an image_processing weitergeleitet.
    """
    ...
    if preprocessing_resolution is not None:
        if preprocessing_resolution[0]< im.shape[0] and preprocessing_resolution[1]< im.shape[1]:
            ...
            logger.warning(f'preprocessing resolution {preprocessing_resolution} ist nicht kleiner als die ursprÃƒÂ¼ngliche BildgrÃƒÂ¶sse {im.shape}. BildgrÃƒÂ¶sse wird nicht verÃƒÂ¤ndert')
    ...
    if len(regionlist)==0:
        logger.info(f"Keine ausgeschnittenen Regionen gefunden. Ev. mÃƒÂ¼ssen die Schwellwerte/Parameter angepasst werden")    
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='''Dieses Kommandozeilenprogramm verlangt ein Dateinamen-Argument "imgfn" (kurz fÃƒÂ¼r image file name). Es fÃƒÂ¼hrt eine Objektlokalisation in diesem Bild durch und extrahiert den Bildausschnitt in den Ausgabepfad (welche mit -o angegeben werden kann). Ein Beispiel zum Ausprobieren ist mitgeliefert:\n
...
formatter_class=argparse.RawTextHelpFormatter)
    parser.add_argument('imgfns',nargs='+',help='Bilddateien (*.png, *.jpg u.ÃƒÂ¤.) mit einer Aufnahme von Objekten vor schwarzem Hintergrund')
    ...
    parser.add_argument('-o','--output',help='Ausgabepfad fÃƒÂ¼r extrahierte Objekte',default='./tmp')
    parser.add_argument('-fr','--fraction_of_rows_to_remove',help='Dieser Zahl (zwischen 0 und 1) definiert die HÃƒÂ¶he des Randbandes als Bruchteil der BildhÃƒÂ¶he. Objekte, deren Bounding-Box teilweise dieses Randband berÃƒÂ¼hren, werden nicht extrahiert. Damit kÃƒÂ¶nnen objekte, die am Rand des Blickfeldes der Kamera auftreten, verworfen werden. Diese Objekte sind oft unvollstÃƒÂ¤ndig und daher fÃƒÂ¼r einen Bildklassifikationstrainingsdatensatz nicht geeignet',default=0.1)
    parser.add_argument('-fc','--fraction_of_cols_to_remove',help='Diese Zahl (zwischen 0 und 1) definiert die Anzahl Pixelspalten am linken und rechten Bildrand, welche vom Kamerabild abgeschnitten werden. Dort zeigen sich oft helle Bereiche ausserhalb des FÃƒÂ¶rderbandes. Diese kÃƒÂ¶nnen als Objekte (falsch-)interpretiert werden, was oft unerwÃƒÂ¼nscht ist.',default=0.13)
    parser.add_argument('-es','--erosion_size',help='Diese Zahl gibt die GrÃƒÂ¶sse der Erosionsmaske an, welche auf die Bildmaske angewendet wird. Anschliessend wird eine Dilatation durchgefÃƒÂ¼hrt- siehe --dilation_size',default=5)
    parser.add_argument('-ds','--dilation_size',help='Diese Zahl gibt die GrÃƒÂ¶sse der Dilatationsmakse an, welche auf die Bildmaske angewendet wird. Davor wird eine Erosion durchgefÃƒÂ¼hrt- siehe --erosion_size.',default=50)
    parser.add_argument('-mpx','--minimum_number_of_pixels',help='Die kleinstmÃƒÂ¶gliche Anzahl Pixel einer Region, fÃƒÂ¼r die noch ein Ausschnitt generiert wird.',default=10000)

    parser.add_argument('-vth','--value_threshold',help='Vorzeichenbehafteter Schwellwert fÃƒÂ¼r Hintergrundpixel im Value-Kanal des Bildes, ein Schwellwert im Bereich [-255,255]. Bei positivem Schwellwert werden Pixel mit Werten nicht grÃƒÂ¶sser als der value_threshold als Hintergrundpixel angesehen. Bei einem negativen Schwellwert werden Pixel mit Werten nicht kleiner als der Absolutwert des value_threshold als Hintergrundpixel angesehen.',default=100)
    parser.add_argument('-hth','--hue_threshold',help='Schwellwert fÃƒÂ¼r Hintergrundpixel im Hue-Kanal des Bildes, ein Schwellwert im Bereich [0,255]. Werte nicht grÃƒÂ¶sser als der hue_threshold werden als Hintergrundpixel angesehen. Negative Werte sind mÃƒÂ¶glich, siehe --value_threshold.',default=None)
    parser.add_argument('-sth','--saturation_threshold',help='Schwellwert fÃƒÂ¼r Hintergrundpixel im Saturation-Kanal des Bildes, ein Schwellwert im Bereich [0,255]. Werte nicht grÃƒÂ¶sser als der saturation_threshold werden als Hintergrundpixel angesehen. Negative Werte sind mÃƒÂ¶glich, siehe --value_threshold.',default=None)
    parser.add_argument('-fv','--fill_value',help='Pixelwert (zwischen 0 und 255), mit dem die an den RÃƒÂ¤ndern abgeschnittenen Pixel aufgefÃƒÂ¼llt werden. Siehe auch -fr und -fc',default=0)

    parser.add_argument('-n','--num_cores',help='Anzahl Kerne, die zur Berechnung benÃƒÂ¼tzt werden. So lÃƒÂ¤sst die Rechenzeit reduzieren.',default=1)
    parser.add_argument('-pr','--preprocessing_resolution',help='Reduziere die AuflÃƒÂ¶sung der geladenen Bilder auf dieses Tupel (Anzahl Zeilen,Anzahl Spalten). Eingabe ist Komma-separiert, Klammern werden entfernt',default='1000,1000')
    parser.add_argument('-y','--yes_to_all',action='store_true',help='Keine RÃƒÂ¼ckfragen',default=False)
...
def process_arguments(args):
    ...
    if len(imgfns)==0:
        logger.error(f'Dateien wurden mit folgendem Ausdruck gesucht:{args.imgfns}.\nDabei wurden keine Dateien gefunden! Bitte ÃƒÂ¼berprÃƒÂ¼fen Sie die Eingabe genau.')
